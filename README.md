...
Если видео не воспроизводится на вашем компьютере:

1. Запишите образец видео в файл и откройте его в браузере напрямую. Пример команды записи файла mp4 с отключённым звуком длительностью 4 секунды:
```bash
ffmpeg -i rtsp://<адрес_камеры> -c:v copy -an -t 4 test.mp4
```

2. Если файл не воспроизводится, проверьте, включено ли в настройках браузера аппаратное ускорение графики и поддерживает ли его ваш компьютер.
Убелитесь, что по адресу [chrome://gpu/](chrome://gpu/) в разделе "Video Acceleration Information" присутствуют строки вида "Decode hevc main".

3. Если файл воспроизводится в браузере, проверьте, верно ли указаны кодеки в поле "codecs" в файле конфигурации _config.py.
Например, для видеокамер с поддержкой H.265 может сработать кодек "hev1.1.6.L120.0", а для H.264 - "avc1.42001f".
Точное значение кодека можно получить, например, командой
```bash
