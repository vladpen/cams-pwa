...
Приложение позволяет подключить к каждой камере неограниченное количество клиентов
...
При наличии аппаратного ускорения видео приложение поддерживает протокол H.265 (HEVC),
который воспроизводится в браузерах на основе Chrome/Chromium не ниже 108 версии и в браузере Firefox не ниже 137 версии.
Протокол H.264 воспроизводится во всех браузерах.
...
Камеры и группы можно добавить в приложении с помощью соответствующих пунктов главного меню.
Для доступа к каждой камере пользователю нужно указать её "ключ" (т.е. ключ словаря Config.cameras, заданный в файле конфигурации).
Ключи защищены от случайной утечки через адресную строку браузера или ссылки Deep Link.

Такого ограничения вполне достаточно для работы в локальной сети.
Но если вы хотите опубликовать приложение в интернете,
в качестве меры усиления безопасности рекомендуется добавить дополнительную авторизацию,
например базовую аутентификацию (Basic Auth) с помощью [nginx](https://ru.wikipedia.org/wiki/Nginx).

### SSL сертификат (необязательно)

Для работы PWA (прогрессивного веб приложения) требуется валидный SSL сертификат,
который обеспечивает соединение по защищённому протоколу HTTPS.

Но в некоторых конфигурациях системы, например при проксировании через фронтенд-сервер nginx,
...
[Менеджер сертификатов](chrome://certificate-manager/localcerts/usercerts).
...
Интерфейс и параметры настроек могут сильно отличаться в зависимости от производителя,
...
Родительская папка camera_folder должна соответствовать настройке cameras[key][folder]
...
Если видео не воспроизводится на вашем компьютере, запишите короткий образец видео в файл и откройте его в браузере напрямую.
...
Точное значение кодеков для записанного файла можно получить, например, командой MP4Box (входящей в пакет gpac):
```bash
...
* Для камер со звуком проверьте, не блокируется ли воспроизведение звуковым каналом.
...
Подробное описание и обсуждение приложения: [habr.com/ru/post/715016](https://habr.com/ru/post/715016/)
